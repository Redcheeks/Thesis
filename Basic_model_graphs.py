...
from simulation.models.LIF_model_basic_theory import (
    LIF_Model_basic_theory as LIF_basic,
)  # class handling LIF models.
from descending_drive import cortical_input  # script for creating current input.
import seaborn as sns

""" Used to plot theoretical output for diagram. """
...
def Output_plots(CI: np.array, simulation_data: List[Tuple[Neuron,]]):
    ...
    ax.set_title("Spike Train With 1 doublet")
...
def _main():
    ...
    model_choice = LIF_basic  # Options: LIF_Model1, LIF_Model2
    ...
    neurons_to_simulate = [all_neurons[200]]
    ...
    simulation_results = [
        (
            b,
            model_choice.simulate_neuron(T, DT, neuron=b, Iinj=input),
        )
