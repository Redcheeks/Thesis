...
% Calculate activation times based on the 5th firing within 1 second
for i = 1:length(discharge_times)
    times = discharge_times{i} / fs;
    for j = 1:length(times) - 4
        if times(j + 4) - times(j) <= 1
            activation_times(i) = times(j + 4);
            break;
        end
    end
end
...
ignore_period = 5 * fs; % Convert to samples
